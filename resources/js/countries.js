...
chang_country_btn.addEventListener("click", function(){
    ...
    search_country_element.classList.add("fadeIn");//Sınıf listesine animasyon ekledik.Ülkeleri arama kısmının üzerine geldiğimizde ülkeler görünmektedir üzerine gitmediğimizde ise görünmemektedir.
});

close_list_btn.addEventListener("click", function(){//Oradaki kapat butonuna bir olay dinleyicisi ekledik.Ülkelerin kaybolması için
    search_country_element.classList.toggle("hide");
});

country_list_element.addEventListener("click", function(){//Herhangi bir ülkeye tıkladığımda ona ait istatistikleri getirmesi için bir olay yarattık.
    search_country_element.classList.toggle("hide");
});

// COUNTRY FILTER
//Buradaki olay yalnızca giriş değeri değiştiğinde tetiklenir. 
